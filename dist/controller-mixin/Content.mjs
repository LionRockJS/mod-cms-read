...
export default class ControllerMixinContent extends ControllerMixin {
    ...
    static async list(state) {
        ...
        const prints = pages.map(page => {
            ...
            if (print.tokens.start)
                print.tokens.start = HelperLabel.formatDate(print.tokens.start, language);
            if (print.tokens.end)
                print.tokens.end = HelperLabel.formatDate(print.tokens.end, language);
            return print;
        }).filter(it => (it !== null));
